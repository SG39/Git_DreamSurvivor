...
public class StatusControllerPower : MonoBehaviour
{
    ...
    private int iLevel = PlayerStatusManager.Instance.powerLevel;
    private int iAmount = PlayerStatusManager.Instance.powerAmount;
    private float fFigure = PlayerStatusManager.Instance.powerFigure;
    private List<float> goldList = PlayerStatusManager.Instance.powerGoldList;
    private float fMaxGoldAmount = PlayerStatusManager.Instance.powerGoldMaxAmount;
...
#region 이벤트 트리거 등록 
   ...
   {
        // 이벤트 트리거 컴포넌트 가져오기
        EventTrigger powerTrigger = statusBtn.gameObject.AddComponent<EventTrigger>();
        ...
        powerTrigger.triggers.Add(buttonDownEntry);
...
#region 버튼이벤트시 동작하는 메서드
    ...
    private void UpgradeStatus()
    {
        ...
        if(Main.Instance.PossessionGold >= goldList[iAmount])
        {
            ...
            // 현재 강화 진행도 증가
            iAmount++;
            PlayerStatusManager.Instance.powerAmount = iAmount;
            ...
            if(iAmount == 100)
            {
                // 레벨업
                iLevel++;
                PlayerStatusManager.Instance.powerLevel = iLevel;
                // 진행도 초기화
                iAmount = 0;
                // 레벨에 따른 각 스테이터스 갱신
                this.UpdateStatus();
