...
public class StatusControllerAttackDuration: MonoBehaviour
{
    ...
    private int iLevel = PlayerStatusManager.Instance.attackDurationLevel;
    private int iAmount = PlayerStatusManager.Instance.attackDurationAmount;
    private float fFigure = PlayerStatusManager.Instance.attackDurationFigure;
    private List<float> goldList = PlayerStatusManager.Instance.attackDurationGoldList;
    private float fMaxGoldAmount = PlayerStatusManager.Instance.attackDurationGoldMaxAmount;
...
#region 데이터 표시하기
    ...
    private void SetAllStatusText()
    {
        ...
        Debug.Log($"Null 오류 체크 >>{Math.Ceiling(goldList[iAmount])}");
...
#region 이벤트 트리거 등록 
   ...
   {
        // 이벤트 트리거 컴포넌트 가져오기
        EventTrigger attackDurationTrigger = statusBtn.gameObject.AddComponent<EventTrigger>();
        ...
        attackDurationTrigger.triggers.Add(buttonDownEntry);
